...
const BreakdownBar = () => {
  const [principal, setPrincipal] = useState(4200); // Net merchant payout
  const [taxes, setTaxes] = useState(300); // Platform fee
  const [insurance, setInsurance] = useState(120); // Network fee
  const [hoa, setHOA] = useState(180); // Liquidity provider fee
  const [utilities, setUtilities] = useState(200); // Compliance & settlement cost

  const total = principal + taxes + insurance + hoa + utilities;

  const segments = [
    { label: "Merchant payout", value: principal, color: "bg-[#1e4532]" },
    { label: "Platform fee", value: taxes, color: "bg-[#701a75]" },
    { label: "Network fee", value: insurance, color: "bg-pink-200" },
    { label: "Liquidity provider fee", value: hoa, color: "bg-yellow-300" },
    { label: "Compliance & settlement", value: utilities, color: "bg-orange-400" },
  ];
